...
- **`seed_utils.py`** — `set_seed(seed)`: seeds `random`/NumPy/PyTorch (CPU+CUDA). Fixes a separate real bug: `train.py` seeded the train/val/test *split* but nothing else, so model weight init, `make_random_code_embeddings()`'s Xavier init, `Dropout`, and `DataLoader(..., shuffle=True)`'s batch order all drew from unseeded global RNGs — two runs with identical CLI args (including an identical `--seed`) gave different metrics. `train.py` now calls `set_seed(args.seed)` before every training run, including per-trial inside a `--sweep`. See the top-level README's "Reproducibility" section for verification.
