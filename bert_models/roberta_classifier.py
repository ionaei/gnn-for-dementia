"""
BERT-Variant Ablation: RoBERTa instead of BioClinicalBERT

Purpose:
  Tests whether the BioClinicalBERT model's superior performance is due to its
  clinical pre-training, or whether a general-purpose BERT-like model (RoBERTa-large)
  can achieve comparable results on the same text input.

Model:
  - Uses roberta-large (AutoModelForSequenceClassification)
  - Same input text format as text-only BioClinicalBERT (demographics + diagnoses + temporal info)
  - Fine-tuned on dementia classification task
...
def train_and_evaluate(
    ...
    """Train and evaluate RoBERTa (or other BERT-variant) for dementia classification."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print(f"BERT-Variant Ablation: Using {model_name}")
    ...
    if use_wandb and WANDB_AVAILABLE:
        wandb.init(
            ...
            name=f"{model_name.replace('/', '_')}_v1",
            config={
    ...
    checkpoint_name = model_name.replace("/", "_").replace("-", "_")
    best_checkpoint = os.path.join(checkpoint_dir, f"best_{checkpoint_name}_classifier.pt")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="BERT-Variant Ablation: Train RoBERTa (or other model) for dementia prediction."
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="HuggingFace model name (e.g., roberta-large, bert-base-uncased)",
    )
    parser.add_argument(
